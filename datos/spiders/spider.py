...
class DatosSpider(CrawlSpider):
	...
	start_urls = [
	    'https://datos.gov.co/browse?limitTo=datasets&sortBy=newest&utf8=&page=965']
	rules = {
		# Para cada item
		Rule(LinkExtractor(allow=(), restrict_xpaths=('/html/body/div[2]/div/div[5]/div/div[3]/div[3]/div[3]/div[1]/a[10]'))),
		Rule(LinkExtractor(allow=(), restrict_xpaths=('//h2/a')), callback='parse_item', follow=False)
	}

	def parse_item(self, response):
            time.sleep(5)
            mi_item = DatosItem()
            mi_item['titulo'] = response.xpath('normalize-space(//h1[@class="info-pane-name"]/span/span/text())').extract_first()
            mi_item['descripcion'] = response.xpath('normalize-space(//*[@id="app"]/div/div[2]/div/div/div[1]/div/div[1]/h1/span/span)').extract_first()
            mi_item['actualizado'] = response.xpath('normalize-space(//*[@id="app"]/div/div[3]/div[1]/section/div[2]/dl/div[1]/div/div[1]/div/dd/text())').extract_first()
